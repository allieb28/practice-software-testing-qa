...
3. Suggested folder structure inside the collection, mirroring the Swagger tags:
   ...
   - Users
   - Orders / Invoices
4. Once the collection has requests and tests, export it here (File > Export in Postman) so it's
   tracked in version control alongside the environment.

## Files

- `environment.template.json` - starting point for the Postman environment; import and fill in.
- `Toolshop API two end-to-end flows.postman_collection.json` - exported collection covering auth,
  cart lifecycle, and checkout/invoice flows with scripted assertions.
